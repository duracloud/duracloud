 ...
 * - hash already stored in properties
...
public class HashFinderWorker implements Runnable {
    ...
    private String getStoredHash() {
        Map<String, String> properties = getContentProperties();
        if (null == properties) {
            throwRuntime("properties are null");
        }
        ...
        if (null == hash) {
            throwRuntime("hash properties element not found");
        }
        return hash;
    }
    ...
    private String getGeneratedHash() {
        ...
        // hash-properties succeeded &&
        // the generated hash does not match the stored properties hash.
        boolean hashesMatch =
            hashProperties == null ? true : hashProperties.equals(hashGenerate);
        
        int tries = 0;
        final int maxTries = 3;
        while (!hashesMatch && tries < maxTries) {
            hashGenerate = doGetGeneratedHash();
            if (null != hashGenerate && null != hashProperties) {
                hashesMatch = hashGenerate.equals(hashProperties);
