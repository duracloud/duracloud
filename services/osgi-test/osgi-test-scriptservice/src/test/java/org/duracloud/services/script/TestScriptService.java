...
public class TestScriptService {
    ...
    public void testServiceBasics() throws Exception {
        Assert.assertNotNull(scriptService.describe());
        Assert.assertNotNull(scriptService.getServiceProps());
    }
    ...
    public void testNoScripts() throws Exception {
        try {
            scriptService.start();
            Assert.fail("Start should throw when no script is available");
        ...
        try {
            scriptService.stop();
            Assert.fail("Stop should throw when no script is available");
        } catch(ScriptServiceException expected) {
            Assert.assertNotNull(expected);
        }
    }
