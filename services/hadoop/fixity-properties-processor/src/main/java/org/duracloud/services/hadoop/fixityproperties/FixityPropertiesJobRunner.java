 ...
 * Date: Feb 9, 2011
 */
public class FixityPropertiesJobRunner extends JobRunner {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println(
            "Starting Fixity Properties Job with args: " + Arrays.toString(args));

        JobRunner runner = new FixityPropertiesJobRunner();

        InitParamParser paramParser = new InitParamParser();
        Map<String, String> initParams = paramParser.parseInitParams(args);

        JobBuilder jobBuilder = new FixityPropertiesJobBuilder(initParams);
