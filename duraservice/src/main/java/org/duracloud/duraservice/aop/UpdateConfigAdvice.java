...
public class UpdateConfigAdvice
    ...
    private final Logger log = LoggerFactory.getLogger(UpdateConfigAdvice.class);

    protected static final int SERVICE_ID_INDEX = 0;

    protected static final int DEPLOYMENT_ID_INDEX = 1;

    private JmsTemplate updateConfigJmsTemplate;
    ...
    public void afterReturning(Object returnObj,
        ...
        publishUpdateConfigEvent(createServiceMessage(methodArgs));
    }

    private void publishUpdateConfigEvent(ServiceMessage updateConfigEvent) {
        getUpdateConfigJmsTemplate().convertAndSend(getDestination(), updateConfigEvent);
    }

    private ServiceMessage createServiceMessage(Object[] methodArgs) {
        String serviceId = getServiceId(methodArgs);
        String serviceHost = getDeploymentId(methodArgs);

        ServiceMessage msg = new ServiceMessage();
        msg.setServiceId(serviceId);
        msg.setDeploymentId(serviceHost);
    ...
    private void doLogging(Object returnObj,
        ...
        log.debug(pre0 + "advice: publish to updateConfig topic");
    ...
    public JmsTemplate getUpdateConfigJmsTemplate() {
        return updateConfigJmsTemplate;
    }

    public void setUpdateConfigJmsTemplate(JmsTemplate updateConfigJmsTemplate) {
        this.updateConfigJmsTemplate = updateConfigJmsTemplate;
    }
