...
public class ServiceManager {
	...
	// The primary services host, generally deployed on the primary user
	// instance
	...
	// Context for connection to the services admin at the primary service
	// instance
	...
	/**
	 * Initializes the service manager with the provided XML document so that
	 * the service manager is able to connect to all services service compute
	 * instances, and user storage.
	 * 
	 * @param configXml
	 *            the xml used to initialize the service manager
	 */
	public void configure(InputStream configXml) {
		...
		ServiceComputeInstance serviceComputeInstance = serviceComputeInstanceUtil
				.createComputeInstance(primaryHost, primaryServicesAdminPort,
						primaryServicesAdminContext, PRIMARY_HOST_DISPLAY);
	...
	private void checkConfigured() {
		if (serviceStore == null) {
			throw new ServiceException(
					"The Service Manager must be initialized "
	...
	private void parseManagerConfigXml(InputStream xml) {
		try {
			...
			Element primaryInstance = servicesConfig
					.getChild("primaryServiceInstance");
			primaryHost = primaryInstance.getChildText("host");
			primaryServicesAdminPort = primaryInstance
					.getChildText("servicesAdminPort");
			primaryServicesAdminContext = primaryInstance
					.getChildText("servicesAdminContext");
			...
			Element serviceComputeProvider = servicesConfig
					.getChild("serviceCompute");
			serviceCompute = new ServiceCompute();
			String computeProviderType = serviceComputeProvider
					.getChildText("type");
			serviceCompute.setType(ComputeProviderType
					.fromString(computeProviderType));
			serviceCompute.setImageId(serviceComputeProvider
					.getChildText("imageId"));
			Element computeCredential = serviceComputeProvider
					.getChild("computeProviderCredential");
			String serviceComputeUsername = computeCredential
					.getChildText("username");
			String serviceComputePassword = computeCredential
					.getChildText("password");
			serviceCompute.setUsername(serviceComputeUsername);
			serviceCompute.setPassword(serviceComputePassword);

			// FIXME: These credentials should come from a 'service-store'
			// config element.
	...
	/*
	 * Reviews the list of content available in the DuraCloud service storage
	 * location and builds a service registry.
	 */
	private void initializeServicesList() throws ContentStoreException {
		...
		storeManager = contentStoreManagerUtil.getContentStoreManager(
				serviceStore, serviceStore.getCredential());
	...
	/*
	 * Retrieves the xml file where all service information is contained and
	 * rebuilds the list of services.
	 */
	private void refreshServicesList() {
		...
		try {
			Content servicesInfoFile = serviceStoreClient.getContent(
					servicesSpaceId, configFileName);

			this.services = ServicesConfigDocument
					.getServiceList(servicesInfoFile.getStream());
	...
	/**
	 * Retrieves a listing of services which are available for deployment along
	 * with the description and user configuration for each service
	 * 
	...
	public List<ServiceInfo> getAvailableServices() {
		...
		for (ServiceInfo service : services) {
			...
			if (!underMaxDeployments(service.getId(), service
					.getMaxDeploymentsAllowed())) {
	...
	private ServiceInfo populateService(ServiceInfo service) {
		return serviceConfigUtil.populateService(service,
				serviceComputeInstances, userStore, primaryHost);
	}
	...
	/**
	 * Attempts to deploy the service with the given ID the the server at the
	 * given host. The service is configured based on the provided user
	 * configuration as well as a set of system configuration values.
	 * 
	 * @param serviceId
	 *            the ID of the service to be deployed
	 * @param serviceHost
	 *            the server host on which the service is to be deployed
	 * @param userConfigVersion
	 *            version of the user configuration
	 * @param userConfig
	 ...
	 *             if there is no services compute instance at serviceHost
	 */
	public int deployService(int serviceId, String serviceHost,
			String userConfigVersion, List<UserConfigModeSet> userConfigModeSets)
			throws NoSuchServiceException,
		...
		if (!underMaxDeployments(serviceId, maxDeployments)) {
			String error = "Service not deployed, the maximum deployment "
					+ "limit for this service " + maxDeployments
					+ " has been reached";
			throw new ServiceException(error);
		}

		ServiceComputeInstance computeInstance = getServiceHost(serviceId,
				serviceHost);
		...
		Map<String, String> serviceConfig = createServiceConfig(
				userConfigModeSets, systemConfig);
		...
		try {
			...
			Content content = serviceStoreClient.getContent(serviceStore
					.getSpaceId(), contentId);
			...
			ServicesAdminClient servicesAdmin = computeInstance
					.getServicesAdmin();
			HttpResponse response = servicesAdmin.postServiceBundle(contentId,
					serviceStream, length);
			...
			response = servicesAdmin
					.postServiceConfig(contentId, serviceConfig);
			...
			String error = "Unable to deploy service " + serviceId + " to "
					+ serviceHost + " due to error: " + e.getMessage();
			log.error(error);
			throw new ServiceException(error, e);
		}

		return storeDeployedService(srvToDeploy, computeInstance.getHostName(),
				userConfigModeSets, systemConfig);
	}

	private void waitUntilDeployed(String contentId,
			ServicesAdminClient servicesAdmin) throws Exception {
		int waitMillis = 2000;
		int maxLoops = 5;
	...
	private void waitUntilConfigured(String configId,
			Map<String, String> expected, ServicesAdminClient servicesAdmin)
			throws Exception {
	...
	private void checkUserConfigVersion(ServiceInfo service,
		...
		if (!userConfigVersion.equals(currentConfigVersion)) {
			String error = "User config version " + userConfigVersion
					...
					+ ". These versions must match in order "
	...
	/*
	 * Removes entries in the map with null keys. Updates map entries with null
	 * values to have an empty string value.
	 */
	...
	/*
	 * Attempts to verify that the service host is valid based on deployment
	 * options configured for this service. If the service is to be deployed on
	 * a new compute host, makes the call to create the new host.
	 * 
	 * @return the service compute instance for this host
	 */
	private ServiceComputeInstance getServiceHost(int serviceId,
			String serviceHost) throws NoSuchServiceException,
			NoSuchServiceComputeInstanceException {
		...
		String error = "Cannot deploy service, the host " + host + " is not "
				+ "a valid deployment location for service with ID "
				+ serviceId;
			...
			for (DeploymentOption option : depOptions) {
				if (option.getLocationType().equals(
					...
					if (option.getState().equals(
						...
						host = createServiceInstance("Service Compute Instance");
					} else {
						throw new ServiceException(error);
					}
				}
		...
		// Existing Host
		ServiceComputeInstance computeInstance = getServiceComputeInstanceByHostName(host);
		if (computeInstance.isLocked()) {
			error = "Cannot deploy service, the host "
					+ host
					+ " has "
	 ...
	 * 
	 * @throws NoSuchServiceException if there is no service in the list with
	 * the given ID
	 */
	...
	/*
	 * Creates a name/value map of user and system config to be used for
				...
				for (UserConfigMode mode : userConfigModeSet.getModes()) {
					for (UserConfig userConfig : mode.getUserConfigs()) {
						if (userConfig instanceof TextUserConfig) {
							...
							serviceConfig.put(textConfig.getName(), textConfig
									.getValue());
							...
							if (value != null) {
								serviceConfig
										.put(singleConfig.getName(), value);
							}
	...
	/*
	 * Attempts to determine the length of content. If the length cannot be
	...
	private long getContentLength(Content content) {
		...
		if (size != null) {
			try {
				...
				log.warn("Unable to determine size: " + size + ", "
						+ content.getId());
				length = 0;
			}
		}
	 ...
	 * configuration values, the completed system configuration, the deployment
	 * host, and a freshly generated deployment ID.
	 * 
	 * @return deployment ID
	 */
	private int storeDeployedService(ServiceInfo serviceToStore,
			String serviceHost, List<UserConfigModeSet> userConfigModeSets,
		...
		for (ServiceInfo deployedService : deployedServices) {
			if (service.getId() == deployedService.getId()) {
				if (deployedService.getUserConfigVersion().equals(
		...
		if (existingServiceDeployment != null) { // Add to deployment list
			List<Deployment> deployments = existingServiceDeployment
					.getDeployments();
	...
	/**
	 * Gets a service which has been deployed. This provides the full set of
	 * userconfig options as well as the selected options currently in use a
	 ...
	 *             if either service or deployment does not exist
	 */
	public ServiceInfo getDeployedService(int serviceId, int deploymentId)
			throws NoSuchDeployedServiceException {
		checkConfigured();

		ServiceInfo deployedService = findDeployedService(serviceId,
				deploymentId);
	...
	public Map<String, String> getDeployedServiceProps(int serviceId,
			int deploymentId) throws NoSuchDeployedServiceException {
		checkConfigured();

		ServiceInfo deployedService = findDeployedService(serviceId,
				deploymentId);
		Deployment serviceDeployment = findServiceDeployment(deployedService,
				deploymentId);
		...
		ServicesAdminClient servicesAdmin = serviceComputeInstance
				.getServicesAdmin();
		Map<String, String> serviceProps;
		try {
			serviceProps = servicesAdmin.getServiceProps(deployedService
					.getContentId());
		} catch (Exception e) {
	...
	/*
	 * Finds a deployed service in the list based on the service and deployment
	 * IDs
	 */
	 ...
	 * @param deploymentId
	 *            the ID of the service deployment to update
	 * @param userConfigVersion
	 *            version of the user configuration
	 * @param userConfigModeSet
	 *            the updated user configuration for this service deployment
	 * @throws NoSuchDeployedServiceException
	 *             if either service or deployment does not exist
	 */
	public void updateServiceConfig(int serviceId, int deploymentId,
			String userConfigVersion, List<UserConfigModeSet> userConfigModeSets)
			throws NoSuchDeployedServiceException {
		checkConfigured();
		...
		try {
			ServicesAdminClient servicesAdmin = getServicesAdmin(serviceDeployment
					.getHostname());
	...
	private void checkResponse(HttpResponse response, String error) {
		...
		if (statusCode != HttpURLConnection.HTTP_OK) {
			String err = error + " Services Admin response code: " + statusCode
	...
	/**
	 * Gets a service. Includes user configuration options, description, etc. as
	 * well as all deployments of the service. Does not guarantee that the
	 * service is available for deployment.
	 * 
	 * @param serviceId
	 ...
	 */
	public ServiceInfo getService(int serviceId) throws NoSuchServiceException {
		checkConfigured();
		refreshServicesList();
	...
	public void undeployAllServices() {
		...
		List<DeploymentInstance> deployments = new ArrayList<DeploymentInstance>();
		for (ServiceInfo service : deployedServices) {
			for (Deployment deployment : service.getDeployments()) {
	...
	public void undeployService(int serviceId, int deploymentId)
		...
		try {
			...
			HttpResponse response = servicesAdmin
					.deleteServiceBundle(contentId);
			...
			String error = "Unable to undeploy service " + serviceId + " from "
					+ serviceHost + " due to error: " + e.getMessage();
			log.error(error);
			throw new ServiceException(error, e);
		}
	...
	private ServicesAdminClient getServicesAdmin(String instanceHost)
			throws NoSuchServiceComputeInstanceException {
		ServiceComputeInstance instance = getServiceComputeInstanceByHostName(instanceHost);
		return instance.getServicesAdmin();
	}

	/**
	 * Locks a service compute instance, meaning that no further services should
	 * be deployed on this instance until the lock is removed.
	 * 
	 * @param instanceHost
	 *            the host name of the compute instance to lock
	 * @throws NoSuchServiceComputeInstanceException
	 *             if no such compute instance exists
	 */
	 ...
	 */
	private ServiceComputeInstance getServiceComputeInstanceByHostName(
			String hostName) throws NoSuchServiceComputeInstanceException {
