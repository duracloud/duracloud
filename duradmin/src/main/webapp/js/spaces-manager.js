...
$(function() {
     ...
     * sequentially in the order they were added. Once a handler returns true,
     * the rest of the handlers are ignored.
     */
    HistoryManager = (function() {
        ...
        var buildStateFromUrl = function(location) {
            ...
            if (pathname) {
                ...
                var subpathname = pathname
                        .substring(index + contextPath.length);

                var first = subpathname.indexOf("/");
                if (first > 0) {
                    state.storeId = subpathname.slice(0, first);
                    var second = first
                            + subpathname.substring(first + 1).indexOf("/");
                    if (second > first) {
                        state.spaceId = subpathname
        ...
        var instance = {
            ...
            /**
             * Adds a change handler to the list of handlers. Handler functions
        ...
        if ($.browser['msie']) {
            ...
            // This is a solution to the a problem related to differences in the
            // way
            // the browser handles popstate on page load. Chrome and Safari
            // issue a
            // 'popstate' event on page load; firefox does not
            // cf
            // http://stackoverflow.com/questions/6421769/popstate-on-pages-load-in-chrome
            // UPDATE: 4/15/2014: It would seems that webkit now handles
            // popstate event on
...
(function() {
    ...
    /**
     * The base pane defines functions that are common to some or all of the
     * panel widgets.
     */
    $
            .widget(
                    "ui.basepane",
                    {
                        ...
                        /**
                         * This method adds uniqueness to a state object to
                         * ensure that when the state is pushed onto the history
                         * stack it causes a pushstack event to fire. If no such
                         * uniqueness occurs, the event will not fire and the
                         * content will not be updated. For example, clicking on
                         * a space that is already loaded, will not be refreshed
                         * unless the state object is made unique.
                         */
                        ...
                        _isObjectAlreadyDisplayedInDetail : function(objectId) {
                            return (this._storeId + "/" + objectId == $(
                                    "#detail-pane .object-id").html());
                        },
                        ...
                        _configureLayout : function() {
                            ...
                            if (layoutOptions) {
                                this._layout = $(this.element).layout(
                                        layoutOptions);
                            }
                        },
                        ...
                        _isReadOnly : function(/* space or contentItem obj */obj) {
                            return obj.callerAcl != "WRITE"
                                    || this._isReadOnlyStorageProvider();
                        },
                        ...
                        _deleteContentItem : function(evt, contentItem) {
                            ...
                            dc.store.DeleteContentItem(contentItem, {
                                ...
                                failure : function(message) {
                                    dc.done();
                                    alert("failed to delete contentItem: "
                                            + message);
                                },
                            });
                        },
                         ...
                         * @param contentItem
                         * @param callback -
                         *            an object with an onProceed() function
                         *            that is called if the content item exists
                         *            and the user has said continue with the
                         *            operation or the content item doesn't
                         *            exist.
                         */
                        _checkIfContentItemExists : function(contentItem,
                                callback) {
                            dc.store
                                    .CheckIfContentItemExists(
                                            contentItem,
                                            {
                                                ...
                                                failure : function(message) {
                                                    alert("check for existing content item failed: "
                                                            + message);
                                                }
                                            });
                        },
                        ...
                        _handleCopyClick : function(dialog, contentItems) {
                            ...
                            var form = $("form", d), destStoreId = $(
                                    "#destStoreId", d).val(), destSpaceId = $(
                                    "#spaceId", d).val(), contentId = $(
                                    "#contentId", d).val();
                            navigateToCopy = $("#navigateToCopy", d).is(
                                    ":checked"), deleteAfterCopy = $(
                                    "#deleteAfterCopy", d).is(":checked"),
                        ...
                        _copyContentItems : function(evt, contentItems) {
                            ...
                            d
                                    .dialog({
                                        ...
                                        open : function() {
                                            var theOther = this;
                                            var destStoreIdField = $(
                                                    "#destStoreId", theOther);
                                            var contentIdField = $(
                                                    "#contentId", theOther);
                                            var spaceSelect = $("#spaceId",
                                                    theOther);
                                            var loadWriteableSpaces = function(
                                                    storeId, spaceId) {
                                                dc.store
                                                        .GetSpaces(
                                                                storeId,
                                                                true,
                                                                {
                                                                    success : function(
                                                                            spaces) {
                                                                        var selectedSpaceId = spaceId ? spaceId
                                                                                : spaceSelect
                                                                                        .val();
                                                                        spaceSelect
                                                                                .children()
                                            ...
                                            // available)
                                            // load list of spaces
                                            destStoreIdField.change(function(
                                                    evt) {
                                                var dest = this;
                                                loadWriteableSpaces($(dest)
                                                        .val());
                                            });
                                            ...
                                            $.validator
                                                    .addMethod(
                                                            ...
                                                            function(value,
                                                                    element) {
                                                                return !(destStoreIdField
                                                                        .val() == contentItems[0].storeId
                                                                        && spaceSelect
                                                                                .val() == contentItems[0].spaceId && contentItems[0].contentId == value);
                                                            },
                                            ...
                                            $("#storeId", theOther).val(
                                                    sourceStoreId);
                                            destStoreIdField.val(sourceStoreId);
                                            var navigateToCopy = $(
                                                    "#navigateToCopy", theOther);
                                            var multiple = contentItems.length > 1;
                                            if (multiple) {
                                                contentIdField.closest("li")
                                                        .hide();
                                                navigateToCopy.closest("li")
                                                        .hide();
                                            } else {
                                                contentIdField.closest("li")
                                                        .show();
                                            ...
                                            loadWriteableSpaces(sourceStoreId,
                                                    first.spaceId);
     ...
     * the detail panes, dialogs, spaces and content item list widgets as well
     * as other functions that coordinate the various panels.
     */
    $
            .widget(
                    "ui.spacesmanager",
                            ...
                            .extend(
                                    ...
                                    $.ui.basepane.prototype,
                                    {
                                        ...
                                        _init : function() {
                                            ...
                                            this._spacesListPane = $('#'
                                                    + this._spacesListViewId,
                                                    this.element);
                                            this._spacesListPane
                                                    .spaceslistpane({
                                            ...
                                            this._contentItemListPane = $(
                                                    '#'
                                                            + this._contentItemListViewId,
                                                    this.element);
                                            ...
                                            this._spacesListPane
                                                    .bind(
                                                            "itemRemoved",
                                                            function(evt, state) {
                                                                if (that._detailManager
                                                                        .isSpaceDetailDisplayed()) {
                                                                    that._detailManager
                                                                            .showSpaces();
                                                                }
                                                            });
                                                    ...
                                                    .bind(
                                                            ...
                                                            function(evt, state) {
                                                                if (that._detailManager
                                                                    ...
                                                                    var selected = that._contentItemListPane
                                        ...
                                        _initHistoryHandlers : function() {
                                                    ...
                                                    .addChangeHandler(function(
                                                        ...
                                                        if (params['spaceId']
                                                            ...
                                                            var contentItems = that._contentItemListPane
                                                                    .contentitemlistpane("selectedContentItems");
                                                            that._detailManager
                                                                    .showMultiContentItems(contentItems);
                                                            return true;
                                                        }
                                                    });
                                            ...
                                            HistoryManager
                                                    .addChangeHandler(function(
                                                            params) {
                                                        if (params['multi']) {
                                                            var spaces = that._spacesListPane
                                                                    .spaceslistpane("selectedSpaces");
                                                            that._detailManager
                                                                    .showMultiSpaces(spaces);
                                                            return true;
                                                        }
                                                    });
                                                                        ...
                                                                        if (space == undefined) {
                                                                            ...
                                                                            $(
                                                                                    ...
                                                                                    .selectablelist(
                                                                                            ...
                                                                                            space.spaceId);
                                                                            that._spacesListPane
                                                                                    .spaceslistpane("scrollToCurrentSpace");
                                                                        }
                                        ...
                                        _initAddSpaceDialog : function() {
                                            ...
                                            // launcher
                                            $('.add-space-button', this.element)
                                                    .live(
                                                            "click",
                                                            function(evt) {
                                                                that._addSpaceDialog
                                                                        .dialog("open");
                                                            });

                                            $('#add-space-help-content')
                                                    .expandopanel({
                                        ...
                                        _initializeDetailManager : function() {
                                            ...
                                            this._detailManager = DetailManager = (function() {
                                                return {
                                                    ...
                                                    isContentItemDisplayed : function(
                                                            contentItem) {
                                                    ...
                                                    showSpaces : function(
                                                        ...
                                                        var storeId = (params ? params.storeId
                                                                : null);
                                                        if (storeId == null) {
                                                            storeId = that
                                                                    .getStoreId();
                                                    ...
                                                    showMultiSpaces : function(
                                                            spaces) {
                                                        currentlyDisplayedDetail = "spacesMulti";

                                                        that._detailPane
                                                                .replaceContents($(
                                                                        "#spaceMultiSelectPane")
                                                                        .clone());
                                                        that._detailPane
                                                                .spacesmultiselectdetail({
                                                                    storeId : that
                                                                            .getStoreId()
                                                                });
                                                        that._detailPane
                                                    ...
                                                    showMultiContentItems : function(
                                                            contentItems) {
                                                        currentlyDisplayedDetail = "contentMulti";

                                                        that._detailPane
                                                                .replaceContents($(
                                                                        "#contentItemMultiSelectPane")
                                                                        .clone());
                                                        var space = that._spacesListPane
                                                                .spaceslistpane("currentSpace");
                                                        that._detailPane
                                                                .contentmultiselectdetail({
                                                                    storeId : that
                                                                            .getStoreId(),
                                                                    readOnly : that
                                                                            ._isReadOnly(space)
                                        ...
                                        _clearContents : function() {
                                            $("#content-item-list-status")
                                                    .fadeOut();
                                            $("#content-item-list")
                                                    .selectablelist("clear",
                                                            false);
                                            $("#content-item-list-view").find(
                                                    "button,a,input").fadeOut();
                                        ...
                                        _getStoreType : function(storeId) {
                                            ...
                                            return 'no provider found with id = '
                                                    + storeId;
                                        },
                                        ...
                                        _initStoreSelectbox : function() {
                                            ...
                                            $("#" + this._PROVIDER_SELECT_ID)
                                                    .flyoutselect(options)
                                                    .bind(
                                                            "changed",
                                                            function(evt, state) {
                                                                dc
                                        ...
                                        setStoreId : function(storeId) {
                                            $("#" + this._PROVIDER_SELECT_ID,
                                                    this.element).flyoutselect(
                                                    "setValueById", storeId,
                                                    false);
                                        },
                                        ...
                                        _loadSpace : function(params,
                                            ...
                                            if (params.snapshot) {
                                                func = that
                                                        ._loadSnapshot(params);
                                            } else {
                                                func = that._loadSpaceInternal(
                                                        params, showDetail);
                                            }

                                            return $.when(
                                                    that.loadSpaces(params),
                                                            ...
                                                            function(snapshot) {
                                                                ...
                                                                that
                                                                        ._displaySnapshot(
                                                                                ...
                                                                                params);
                                                                that
                                                                        ._loadSnapshotContentItems(snapshot);
                                        ...
                                        _loadSpaceInternal : function(params,
                                                showDetail) {
                                            var that = this;
                                            dc.busy("Loading...", {
                                                modal : true
                                            });
                                            retrieveSpace = dc.store
                                                    .GetSpace2(params)
                                                    .success(
                                                            function(data) {
                                        ...
                                        _loadContentItem : function(params) {
                                            ...
                                            var space = that._contentItemListPane
                                                    .contentitemlistpane("currentSpace");

                                            var loadSpace;
                                            if (space.spaceId == params.spaceId
                                                    && space.storeId == params.storeId) {
                                                loadSpace = $.Deferred()
                                                        .resolve();
                                            } else {
                                                loadSpace = that._loadSpace(
                                                        params, false);
                                            }
                                        ...
                                        _loadSnapshotContentItems : function(
                                                snapshot) {
                                            var that = this;
                                            that._contentItemListPane
                                                    .replaceContents($(
                                                            "#snapshotItemListPane")
                                                            .clone());
                                            that._contentItemListPane
                                                    .snapshotitemlistpane();
                                            that._contentItemListPane
                                        ...
                                        loadSpaces : function(optionalParams) {
                                            ...
                                            $("#provider-logo").addClass(
                                                    this._getStoreType(this
                                                            .getStoreId())
                                                            + '-logo');
                                                    ...
                                                    .layout(
                                                            {
                                                                ...
                                                                west__minSize : 260,
                                                                west__paneSelector : "#"
                                                                        + that._spacesListViewId,
                                                                center__paneSelector : "#"
                                                                        + that._contentItemListViewId,
                                                                center__onresize : function() {
                                                                    that._contentItemListPane
                                                                            .contentitemlistpane("resizeAll");
                                                                }
                                                            });
                                        },
                                    }));
    ...
    $.widget("ui.spaceslistpane", $.extend({}, $.ui.basepane.prototype, {
        ...
        _initFilter : function() {
            ...
            $(".dc-item-list-filter", this.element).bind("keyup",
                    $.debounce(500, function(evt) {
        ...
        setCurrentById : function(spaceId) {
            this._spacesList.selectablelist("setCurrentItemById", spaceId,
                    false);
        },
                    ...
                    if (currentItem) {
                        ...
                        if (spaceId) {
                            newState = that._createUniqueStateObject({
                                ...
                                spaceId : spaceId,
                                snapshot : (currentItem.data.snapshot ? true
                                        : false)
                            });
        ...
        addSpaceToList : function(space) {
            ...
            this._spacesList.selectablelist('addItem', node, space, false,
                    disabled);
        },
        ...
        load : function(spaces, storeId) {
            ...
            $.each(this._spaces, function(i, space) {
                if (!filter
                        || space.spaceId.toLowerCase().indexOf(
                                filter.toLowerCase()) > -1) {

                    if (!snapshotDividerAdded && space.snapshot) {
                        that._spacesList.selectablelist("addDivider",
                                "Snapshots");
        ...
        scrollToCurrentSpace : function() {
            ...
            if (current != null && current != undefined && current.data != null
                    && current.data != undefined) {
                this._spacesList.closest(".dc-item-list-wrapper").scrollTo(
                        current.item);
            }
        },
    ...
    /**
     * This widget defines the content list pane which includes the selected
     * list, filter box, navigation links and related buttons.
     */
    $
            .widget(
                    "ui.contentitemlistpane",
                    $
                            .extend(
                                    {},
                                    $.ui.basepane.prototype,
                                    {
                                        ...
                                        _init : function() {
                                            ...
                                            $(this.element)
                                                    .bind(
                                                            ...
                                                            function(evt, state) {
                                                                that
                                                                        ._handleContentListStateChangedEvent(
                                                                                evt,
                                                                                state);
                                                            });
                                        ...
                                        _toggleCheckAllContentItems : function(
                                                checked) {
                                        ...
                                        _handleContentListStateChangedEvent : function(
                                                evt, state) {
                                            var that = this;
                                            ...
                                            try {
                                                ...
                                                if (length == 0) {
                                                    that
                                                            ._toggleCheckAllContentItems(false);
                                                    if (currentItem) {
                                                        var contentId = $(
                                                                currentItem.item)
                                                                .attr("id");
                                                    ...
                                                    if (length == 1) {
                                                        ...
                                                        var contentItems = this._contentItemListPane
                                                                .contentitemlistpane("selectedContentItems");
                                                        this._detailManager
                                                                .showMultiContentItems(contentItems);
                                                    }
                                        ...
                                        selectedContentItems : function() {
                                            var that = this;
                                            var contentItems = this._getList()
                                                    .selectablelist(
                                                            "getSelectedData");
                                            $
                                                    .each(
                                        ...
                                        _reloadContents : function(spaceId,
                                                marker, handler, message) {
                                            this._getList().selectablelist(
                                                    "clear");
                                                            ...
                                                            spaceId,
                                                            {
                                                                ...
                                                                success : function(
                                                                    ...
                                                                    if (space == undefined
                                                                            || space == null) {
                                                                        that
                                                                                ._showContentItemListStatus("Error: space not found.");
                                                                    } else {
                                                                        handler(space);
                                                                    }
                                                                },
                                        ...
                                        _addContentItemsToList : function(space) {
                                            var that = this, readOnly = this
                                                    ._isReadOnly(space);

                                            $.each(space.contents, function(i,
                                                    value) {
                                        ...
                                        _addContentItemToList : function(
                                        ...
                                        _showContentItemListStatus : function(
                                                text) {
                                            var contentItemListStatus = $(
                                                    "#content-item-list-status",
                                                    this.element);
                                            if (!text) {
                                                contentItemListStatus.fadeOut(
                                                        "fast").html('');
                                            } else {
                                                contentItemListStatus
                                                        .html(text).fadeIn(
                                                                "slow");
                                            }
                                        },
                                        ...
                                        _showMoreHandler : function() {
                                            ...
                                            list = this._getList();
                                            itemData = list
                                                    .selectablelist("lastItemData");
                                            lastItem = list
                                                    .selectablelist("lastItem");
                                            marker = null;
                                                        ...
                                                        .GetSpace(
                                                                ...
                                                                itemData.spaceId,
                                                                {
                                                                    begin : function() {
                                                                        dc
                                                                                .busy(
                                                                                        "Loading more content items...",
                                                                                        {
                                                                                            modal : true
                                                                                        });
                                                                        lastItem
                                                                                .addClass("dc-selectablelist-hl");
                                                                    },
                                                                    success : function(
                                                                            s) {
                                                                        dc
                                                                                .done();
                                                                        that
                                                                                ._addContentItemsToList(s);
                                                                        that
                                                                                ._updateNavigationControls(s);
                                                                    },
                                                                    failure : function(
                                                                            info) {
                                                                        setTimeout(
                                                                                function() {
                                                                                    alert("Failed to retrieve more content items:"
                                                                                            + info);
                                                                                },
                                                                                200);
                                        ...
                                        _updateNavigationControls : function(
                                            ...
                                            listView.find(".dc-show-more-link")
                                                    .remove();
                                            ...
                                            if (space.properties.count == 0) {
                                                ...
                                                totalCount = (this
                                                        ._getFilterText() == '' ? itemCount
                                                        : "?");

                                                if (listCount == 0
                                                        && space.contents.length == 0) {
                                                    statusText = "";
                                                } else {
                                                    statusTxt = "Showing 1 - "
                                                            + listCount
                                            ...
                                            this
                                                    ._showContentItemListStatus(statusTxt);

                                            if (space.contents.length > 199) {
                                                listView
                                                        .find(
                                                                ".dc-item-list-controls")
                                                        .html('')
                                                        .append(
                                                                this
                                                                        ._createShowMoreLink());
                                        ...
                                        _load : function(space) {
                                            ...
                                            var list, listView, readOnly = this
                                                    ._isReadOnly(space);
                                            ...
                                            this
                                                    ._updateNavigationControls(space);
                                                    ...
                                                    .dialog({
                                                        ...
                                                        open : function(e) {
                                                            ...
                                                            buttonPane
                                                                    ...
                                                                    .remove();
                                                            buttonPane
                                                                    .prepend("<div class='dialog-info'><a href='https://wiki.duraspace.org/display/DURACLOUDDOC/How+to+Automatically+Upload+Files+and+Directories+to+DuraCloud' target='_NEW'>Want to make uploads automatic?</a></div>")
                                                        }
    ...
    $.widget("ui.snapshotitemlistpane", $.extend({}, $.ui.basepane.prototype, {
        ...
        _init : function() {
            ...
            $(this.element).find(".dc-item-list-filter").bindEnterKey(
                    function(evt) {
                        that._reloadSnapshotContents(that._snapshot, null,
                                function(snapshot) {
        ...
        _reloadSnapshotContents : function(snapshotId, page, handler, message) {
            ...
            dc.store.GetSnapshotContent(this._snapshot.sourceStoreId,
                    this._snapshot.snapshotId, page, prefix).success(
                    function(data) {
                        handler(data);
                    }).error(function() {
        ...
        _addContentItemToList : function(contentItem) {
            ...
            node = $.fn.create("div");
            node.attr("id", contentItem.contentId).append(content).append(
                    actions);

            var item = this._getList().selectablelist('addItem', node,
        ...
        _showContentItemListStatus : function(text) {
            var contentItemListStatus = $("#content-item-list-status",
                    this.element);
            if (!text) {
        ...
        _createShowMoreLink : function() {
            var link, that = this;
            link = $.fn.create("a");
            link.html('show more').addClass("dc-link").addClass(
                    "dc-show-more-link").click(function() {
        ...
        _showMoreHandler : function() {
            ...
            if (nextPage) {
                ...
                dc.store.GetSnapshotContent(that._snapshot.storeId,
                        that._snapshot.snapshotId, page, prefix).success(
                        function(snapshot) {
        ...
        _updateNavigationControls : function(snapshot) {
            ...
            if (totalCount == 0) {
                ...
                if (listCount == 0 && snapshot.contents.length == 0) {
                    ...
                    statusTxt = "Showing 1 - " + listCount + " of "
                            + totalCount;
                }
            }
            ...
            if (snapshot.contents.length > 199) {
                listView.find(".dc-item-list-controls").html('').append(
                        this._createShowMoreLink());
                list.selectablelist("setFooter", this._createShowMoreLink());
            } else {
                if (totalCount == 0) {
                    list.selectablelist("setFooter", $.fn.create("div").html(
        ...
        load : function(snapshot) {
            ...
            dc.store.GetSnapshotContent(snapshot.sourceStoreId,
                    snapshot.snapshotId, 0, "").success(function(snapshot) {
                that._load(snapshot);
            }).error(function() {
                            ...
                            .extend(
                                    ...
                                    $.ui.basepane.prototype,
                                    {
                                        ...
                                        _setObjectId : function(objectId) {
                                            $(".object-id", this.element).html(
                                                    this._getStoreId() + "/"
                                                            + objectId);
                                        },

                                        _appendToCenter : function(node) {
                                            $(".center", this.element).append(
                                                    node);
                                        },
                                        ...
                                        _loadSnapshotPane : function(space) {
                                            ...
                                            if (this._isSnapshot(space.storeId,
                                                    storeProviders)
                                                    && this
                                                            ._getRestoreId(space) == null) {
                                                var viewerPane = $.fn.create(
                                                        "div").attr("id",
                                                        "snapshot").snapshot({
                                                    open : true,
                                                    space : space
                                                ...
                                                this
                                                        ._appendToCenter(viewerPane);

                                            }

                                        },

                                        _loadPropertiesPane : function(
                                                extendedProperties, /* bool */
                                                readOnly) {
                                            $("#extended-properties",
                                                    this.element).remove();
                                            var viewerPane = this
                                                    ._createPropertiesPane(
                                                            extendedProperties,
                                                            readOnly);
                                            this._appendToCenter(viewerPane);
                                            return viewerPane;
                                        ...
                                        _createMakePublicButton : function(
                                                storeId, spaceId, successFunc) {
                                            var that = this;
                                            var button = $.fn
                                                    .create("button")
                                                    .addClass("featured")
                                                    ...
                                                    .html(
                                                            "Make space publicly readable");

                                            button
                                                    .click(function(evt) {
                                                        that
                                                                ._makeSpacePubliclyReadable(
                                                                        evt,
                                                                        storeId,
                                                                        spaceId,
                                                                        successFunc);
                                        ...
                                        _formatBulkPropertiesUpdateParams : function(
                                                params) {
                                            var data = "";
                                            data += this._formatPropertiesList(
                                                    params.propertiesToRemove,
                                                    "remove");
                                            data += this._formatPropertiesList(
                                                    params.propertiesToAdd,
                                                    "add");
                                            data += this._formatParamList(
                                                    params.tagsToRemove, "tag",
                                                    "remove");
                                            data += this._formatParamList(
                                                    params.tagsToAdd, "tag",
                                                    "add");
                                            return data;
                                        },

                                        _formatPropertiesList : function(list,
                                                fieldNameModifier) {
                                        ...
                                        _formatParamList : function(list,
                                                fieldPrefix, fieldNameModifier) {
                                            var i, list, item, data;
                                            data = "";
                                            for (i = 0; i < list.length; i++) {
                                                item = list[i];
                                        ...
                                        _openContentItemDialog : function(
                                                saveFunction, contentItem) {
                                            ...
                                            d
                                                    ...
                                                    .val(
                                                            contentItem ? contentItem.storeId
                                                                    : "");
                                            d
                                                    .find("input[name=spaceId]")
                                                    .val(
                                                            contentItem ? contentItem.spaceId
                                                                    : "");
                                            d
                                                    .find(
                                                            "input[name=contentId]")
                                                    .val(
                                                            contentItem ? contentItem.contentId
                                                                    : "");
                                            d
                                                    .find(
                                                            "input[name=contentMimetype]")
                                                    .val(
                                                            contentItem ? contentItem.properties.mimetype
                                                                    : "");
                                        ...
                                        _addSpaceProperties : function(spaceId,
                                                name, value, callback) {
                                        ...
                                        _removeSpaceProperties : function(
                                                spaceId, name, callback) {
                                            var data = "properties-name="
                                                    + encodeURIComponent(name);
                                            dc
                                                    .ajax(this
                                        ...
                                        _addSpaceTag : function(spaceId, tag,
                                                callback) {
                                            var data = "tag="
                                        ...
                                        _removeSpaceTag : function(spaceId,
                                                tag, callback) {
                                            var data = "tag="
                                                    + encodeURIComponent(tag);
                                            dc
                                        ...
                                        _addRemoveSpaceProperties : function(
                                                spaceId, params, callback) {
                                            dc
                                                    .ajax(this
                                                            ._createSpacePropertiesCall(
                                                                    spaceId,
                                                                    this
                                                                            ._formatBulkPropertiesUpdateParams(params),
                                                                    "addRemove",
                                                                    callback));
                                        },

                                        _createSpacePropertiesCall : function(
                                                spaceId, data, method, callback) {
                                            var newData = data + "&method="
                                                    + method;
                                            var storeId = this._storeId;
                                            return {
                                                url : "/duradmin/spaces/space?storeId="
                                                        + storeId
                                                        + "&spaceId="
                                        ...
                                        _createContentItemPropertiesCall : function(
                                                spaceId, contentId, data,
                                                method, callback) {
                                            var newData = data + "&method="
                                                    + method;
                                            var storeId = this._storeId;
                                            return {
                                                url : "/duradmin/spaces/content/update-properties?storeId="
                                                        + storeId
                                                        + "&spaceId="
                                                        + encodeURIComponent(spaceId)
                                                        + "&contentId="
                                        ...
                                        _addContentItemProperties : function(
                                                spaceId, contentId, name,
                                                value, callback) {
                                            var data = "properties-name="
                                                    + encodeURIComponent(name)
                                                    + "&properties-value="
                                            ...
                                            dc
                                                    .ajax(this
                                                            ._createContentItemPropertiesCall(
                                                                    spaceId,
                                                                    contentId,
                                                                    data,
                                                                    "addProperties",
                                        ...
                                        _removeContentItemProperties : function(
                                                spaceId, contentId, name,
                                                callback) {
                                            var data = "properties-name="
                                                    + encodeURIComponent(name);
                                        ...
                                        _addContentItemTag : function(spaceId,
                                                contentId, tag, callback) {
                                            var data = "tag="
                                                    + encodeURIComponent(tag);
                                            dc
                                                    .ajax(this
                                        ...
                                        _removeContentItemTag : function(
                                                spaceId, contentId, tag,
                                                callback) {
                                            var data = "tag="
                                                    + encodeURIComponent(tag);
                                            dc
                                        ...
                                        _addRemoveContentItemProperties : function(
                                                spaceId, contentId, params,
                                                callback) {
                                            dc
                                                    .ajax(this
                                                            ._createContentItemPropertiesCall(
    ...
    $
            .widget(
                    "ui.basemultidetailpane",
                    $
                            .extend(
                                    {},
                                    $.ui.basedetailpane.prototype,
                                    {

                                        _init : function() {
                                            $.ui.basedetailpane.prototype._init
                                                    .call(this);
                                        },
                                        ...
                                        _preparePropertiesDialog : function(
                                            ...
                                            if (targetListDataType == "contentItem") {
                                                ...
                                                getFunction = function(ci,
                                                        callback) {
                                                    dc.store.GetContentItem(
                                                            ci.storeId,
                                                            ci.spaceId,
                                                            ci.contentId,
                                                            callback);
                                            ...
                                            this
                                                    ._aggregatePropertiesFromSelection(
                                                            items,
                                                            getFunction,
                                                            {
                                                                success : function(
                                                                        data) {
                                                                    that
                                                                            ._loadPropertiesDialog(
                                                                                    data,
                                                                                    targetListDataType);
                                                                },
                                                                failure : function(
                                                                        text) {
                                                                    alert("unable to load selection:"
                                                                            + text);
                                                                },
                                                            });
                                        ...
                                        _appendToListIfNew : function(newItems,
                                                itemList, equalsFunc) {
                                            var toAppend = [];
                                        ...
                                        _aggregatePropertiesFromSelection : function(
                                                items, getFunction, fcallback) {
                                            var that = this;
                                                                ...
                                                                getFunction(
                                                                        this._item,
                                                                        {
                                                                            success : function(
                                                                                    obj) {
                                                                                propertiesLists
                                                                                        .push(obj.extendedProperties);
                                                                                tagLists
                                                                                        .push(obj.properties.tags);
                                                                                callback
                                                                                        .success();
                                                                            },
                                                                            failure : function(
                                        ...
                                        _removeValueFromList : function(value,
                                                list, equals) {
                                            var i = -1, el;
                                        ...
                                        _loadPropertiesDialog : function(data,
                                            ...
                                            var mp = this
                                                    ._createPropertiesPane(data.properties);
                                            ...
                                            var saveFunction = function() {
                                                ...
                                                if (confirm(msg)) {
                                                    ...
                                                    if (targetListType == "contentItem") {
                                                        ...
                                                        that
                                                                ._bulkUpdateContentProperties(params);
                                                    } else {
                                                        params.spaces = that._spaces;
                                                        that
                                                                ._bulkUpdateSpaceProperties(params);
                                                    }

                                                    d.dialog("close");
                                                    dc
                                                            .busy(
                                                                    "Preparing to perform update...",
                                                                    {
                                                                        modal : true
                                                                    });
                                            ...
                                            var d = that
                                                    ._initializePropertiesDialog(saveFunction);
                                            var center = $(".center", d);
                                            center.append(mp);
                                            center.append(tag);
                                            dc.done();
                                        ...
                                        _bulkUpdateContentProperties : function(
                                                params) {
                                            var that = this;
                                            var job = dc.util
                                                    .createJob("bulk-update-content-properties");
                                                ...
                                                job
                                                        .addTask({
                                                            ...
                                                            execute : function(
                                                                ...
                                                                that
                                                                        ._addRemoveContentItemProperties(
                                                                                citem.spaceId,
                                                                                citem.contentId,
                                                                                params,
                                                                                callback);
                                            ...
                                            job
                                                    .execute(this
                                                            ._createGenericJobCallback("Updating content items: "));
                                        },

                                        _bulkUpdateSpaceProperties : function(
                                                params) {
                                            var that = this;
                                            var job = dc.util
                                                    .createJob("bulk-update-space-properties");
                                            $
                                                    .each(
                                                            params.spaces,
                                                            function(i, space) {
                                            ...
                                            job
                                                    .execute(this
                                                            ._createGenericJobCallback("Updating spaces: "));
                                        },
    ...
    $
            .widget(
                    "ui.spacesmultiselectdetail",
                    $
                            .extend(
                                    {},
                                    $.ui.basemultidetailpane.prototype,
                                    {
                                        _init : function() {
                                            $.ui.basemultidetailpane.prototype._init
                                                    .call(this);
                                            this._initPane();
                                        },
                                        ...
                                        spaces : function(spaces) {
                                            this._spaces = spaces;
                                            this
                                                    ._updateTitle(this._spaces.length);
                                        },
                                        ...
                                        _initPane : function() {
                                            ...
                                            var editPropsButton = $(
                                                    ".add-remove-properties-button",
                                                    this.element);
                                            editPropsButton
                                                    .click(function(evt) {
                                                        that
                                                                ._preparePropertiesDialog("space");
                                                    });
                    ...
                    $
                            .extend(
                                    ...
                                    $.ui.basedetailpane.prototype,
                                    {
                                        ...
                                        _extractSpaceProperties : function(
                                                space) {
                                            var itemCount;
                                            if (space.itemCount == null
                                                ...
                                                itemCount = space.itemCount
                                                        + " <button id='recount'><i class='pre refresh'></i>Recount</button>";
                                            }
                                            ;
                                            ...
                                            var spaceProps = [
                                                    ...
                                                    [
                                                            'Created',
                                                            space.properties.created ] ];

                                            if (space.properties.size) {
                                                spaceProps
                                                        .push([
                                                                'Size',
                                                                space.properties.size ]);
                                            }
                                            ...
                                            var propertiesDiv = this
                                                    ._loadProperties(spaceProps);

                                            if (bitIntegrityResult) {
                                                dc
                                                        .reportOverlayOnClick(
                                                                $(
                                                                        "#report-viewer",
                                                                        propertiesDiv),
                                        ...
                                        _getSpaceMetrics : function(space) {
                                            return dc.store
                                                    .GetStorageReportDetail(
                                                            space.storeId,
                                                            space.spaceId, null);
                                        },

                                        _createMimetypeGraphPanel : function(
                                                space) {
                                            var that = this;
                                            var mimetypePanel = $.fn
                                                    .create("div");
                                            // get metrics ajax call
                                            $
                                                    .when(
                                                            this
                                                                    ._getSpaceMetrics(space))
                                                    .done(
                                                            function(response) {
                                                                $
                                                                        .each(
                                                                                response.metrics.spaceMetrics,
                                                                                function(
                                                                                        i,
                                                                                        metrics) {
                                                    ...
                                                    .GetSpace(
                                                            ...
                                                            space.spaceId,
                                                            {
                                                                success : function(
                                                                        s) {
                                                                    if (that
                                                                            ._isObjectAlreadyDisplayedInDetail(s.spaceId)) {
                                                                        if (s != undefined
                                                                                && s != null) {
                                                                            that
                                                                                    ._extractSpaceProperties(s);
                                                                            if (s.itemCount == null
                                                                                    || parseInt(s.itemCount) < 0) {
                                                                                setTimeout(
                                                                                        function() {
                                                                                            recount = false;
                                                                                            that
                                        ...
                                        load : function(space) {
                                            ...
                                            if (this._isAdmin()
                                                ...
                                                $(".streaming-switch",
                                                        ...
                                                        .onoffswitch(
                                                                {
                                                                    initialState : space.streamingEnabled ? "on"
                                                                            : "off",
                                            ...
                                            if (this._isAdmin()) {
                                                var makePublicButton = this
                                                        ._createMakePublicButton(
                                                                space.storeId,
                                                                space.spaceId);
                                                ...
                                                if (this
                                                        ._isPubliclyReadable(space.acls)) {
                            ...
                            .extend(
                                    ...
                                    $.ui.basedetailpane.prototype,
                                    {
                                        ...
                                        load : function(snapshot) {
                                            ...
                                            this._snapshot = snapshot;
                                            this
                                                    ._setObjectName(snapshot.snapshotId);
                                            this
                                                    ._setObjectId(snapshot.snapshotId);
                                            ...
                                            var props = [
                                                    ...
                                                    [
                                                            "Source Store",
                                                            snapshot.sourceStoreId ],
                                                    [
                                                            "Source Space",
                                                            snapshot.sourceSpaceId ],
                                        ...
                                        _configureRestoreControls : function() {
                                            ...
                                            that._getRestoreButton().disable(
                                                    true);
                                            ...
                                            dc.store
                                                    .GetRestoreBySnapshot(
                                                            that._storeId,
                                                            that._snapshot.snapshotId)
                                                    .success(
                                                            function(restore) {
                                                                that
                                                                        ._enableRestoreLink(restore);
                                                            })
                                                    .error(
                                                            function(jqxhr,
                                                                    textStatus,
                                            ...
                                            dc.store
                                                    ...
                                                    .error(
                                                            function(jqxhr,
                                                                    textStatus,
                                                                    errorThrown) {
                                                                alert("failed to initiate restore: status="
                                                                        + jqxhr.status
                                                                        + "; error thrown="
                                                                        + errorThrown);
                                                            });
    ...
    /**
     * This widget defines detail view to be displayed when multiple content
     * items are selected.
     */
    $
            .widget(
                    "ui.contentmultiselectdetail",
                    $
                            .extend(
                                    {},
                                    $.ui.basemultidetailpane.prototype,
                                    {
                                        ...
                                        _init : function() {
                                            ...
                                            var editButton = $(
                                                    ".edit-selected-content-items-button",
                                                    this.element);
                                            if (readOnly) {
                                                editButton.hide();
                                            } else {
                                                editButton
                                                        .click(function(evt) {
                                                            that
                                                                    ._handleEditButtonClick(evt);
                                                        });
                                            }

                                            var addRemoveProperties = $(
                                                    ".add-remove-properties-button",
                                                    this.element);
                                            if (readOnly) {
                                                ...
                                                addRemoveProperties
                                                        .click(function(evt) {
                                                            that
                                                                    ._preparePropertiesDialog("contentItem");
                                                        });
                                            }

                                            var copyButton = $(
                                                    ".copy-content-item-button",
                                                    this.element);
                                        ...
                                        contentItems : function(contentItems) {
                                            ...
                                            this
                                                    ._updateTitle(this._contentItems.length);
                                        },
                                        _updateTitle : function(count) {
                                            this
                                                    ._setObjectName("{count} content item(s) selected."
                                                            .replace("{count}",
                                                                    count));
                                        },
                                        ...
                                        _handleDeleteButtonClick : function(evt) {
                                            ...
                                            var job = dc.util
                                                    .createJob("delete-content-items");
                                            ...
                                            this
                                                    ._openContentItemDialog(function() {
                                                        ...
                                                        if (form.valid()) {
                                                            dc
                                                                    .busy(
                                                                            "Preparing to update content items...",
                                                                            {
                                                                                modal : true
                                                                            });
                                                            $(
                                                            ...
                                                            var contentItems = that._contentItems;
                                                            var job = dc.util
                                                                    .createJob("update-content-items");
                                                            $
                                                                    .each(
                                                                            contentItems,
                                                                            function(
                    ...
                    $
                            .extend(
                                    ...
                                    $.ui.basedetailpane.prototype,
                                    {
                                        ...
                                        _extractContentItemProperties : function(
                                                contentItem) {
                                        ...
                                        load : function(/* object */contentItem) {
                                            ...
                                            this
                                                    ._setObjectName(contentItem.contentId);
                                            this
                                                    ._setObjectId(contentItem.spaceId
                                                            + "/"
                                                            + contentItem.contentId);
                                            readOnly = this
                                                    ._isReadOnly(contentItem);

                                            $(".download-content-item-button",
                                                    this.element)
                                                    .attr(
                                                            "href",
                                                            dc.store
                                                                    .formatDownloadURL(contentItem));

                                            var deleteContentButton = $(
                                                    ".delete-content-item-button",
                                                    this.element);
                                            deleteContentButton.click(function(
                                                    evt) {
                                                that._deleteContentItem(evt,
                                                        contentItem);
                                            });
                                            if (readOnly) {
                                            ...
                                            $(".durastore-link", this.element)
                                                    .attr(
                                                            "href",
                                                            contentItem.durastoreURL);

                                            this
                                                    ._loadProperties(this
                                                            ._extractContentItemProperties(contentItem));
                                            // load the details panel
                                            var mimetype = contentItem.properties.mimetype;
                                            $(".mime-type .value", this.element)
                                                    .text(mimetype);
                                            $(".mime-type-image-holder",
                                                    this.element)
                                                    .addClass(
                                                            dc
                                                                    .getMimetypeImageClass(mimetype));

                                            var mp = this
                                                    ._loadPropertiesPane(
                                                            contentItem.extendedProperties,
                                                            readOnly);
                                            ...
                                            var editContentItemButton = $(
                                                    ".edit-content-item-button",
                                                    this.element);
                                            editContentItemButton
                                                    .click(function(evt) {
                                                        that
                                                                ._handleEditContentItemClick(evt);
                                                    });
                                            if (readOnly) {
                                        ...
                                        _handleEditContentItemClick : function(
                                                evt) {
                                            var that = this;
                                            this
                                                    ._openContentItemDialog(
                                                            function() {
                                                                var form = $("#edit-content-item-form");
                                                                var data = form
                                                                        .serialize();
                                        ...
                                        _loadPreview : function(contentItem) {
                                            ...
                                            if (imageViewerBaseURL && open) {
                                                ...
                                                viewerURL = dc.store
                                                        .formatDownloadURL(
                                                                ...
                                                                false);
                                                thumbnailURL = dc.store
                                                        .formatGenericThumbnail(contentItem);
                                            }
                                            ...
                                            if (!open && imageViewerBaseURL
                                                ...
                                                warning
                                                        .append(
                                                                "<span>To use the JP2 Viewer you must grant the 'public'"
                                                                        + " group read access to this space.</span>")
                                                        .append(button);
                                            }

                                            $(div).expandopanel("getContent")
                                                    .append(wrapper);
                                            this._appendToCenter(div);
                                        },

                                        _loadVideo : function(contentItem) {
                                            this._loadMedia(contentItem,
                                                    "Watch", "video");
                                        },
                                        ...
                                        _loadMedia : function(contentItem,
                                                title,/* audio or video */type) {
                                                    ...
                                                    .done(
                                                            function(result) {
                                                                ...
                                                                if (streamingHost != null
                                                                    ...
                                                                    viewer
                                                                            .append("<p>Turn on streaming for this space to enable playback.</p>");
                                                                }
                                                            });
                                        },

                                        _writeMediaTag : function(
                                                streamingHost, contentItem) {
                                            setTimeout(
                                                    function() {
                                                        // async necessary to
                                                        // let the DOM update
                                        ...
                                        _createHTML5MediaTag : function(
                                                contentItem, type) {
                                            return type == 'audio' ? this
                                                    ._createHTML5AudioTag(contentItem)
                                                    : this
                                                            ._createHTML5VideoTag(contentItem);
                                        },

                                        _createHTML5AudioTag : function(
                                                contentItem) {
                                            return $.fn.create("audio").attr(
                                                    "src",
                                                    contentItem.viewerURL)
                                                    .attr("loop", "false")
                                                    .attr("preload", "false")
                                                    .attr("controls", "true");
                                        },

                                        _createHTML5VideoTag : function(
                                                contentItem) {
                                            return $.fn.create("video").attr(
                                                    "src",
                                                    contentItem.viewerURL)
                                                    .attr("loop", "false")
                                                    .attr("preload", "false")
