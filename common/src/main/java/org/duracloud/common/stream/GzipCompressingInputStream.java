 ...
 * DeflateInputStream but adds GZIP-header and trailer See GzipOutputStream for
 * details.
 * 
 * @author mwyraz
 * @see http://stackoverflow.com/questions/11036280/compress-an-inputstream-with
 *      -gzip
 * 
 */
public class GzipCompressingInputStream extends SequenceInputStream {
    ...
    protected static class StatefullGzipStreamEnumerator
        ...
        static final byte[] GZIP_HEADER = new byte[] { (byte) GZIP_MAGIC, // Magic
                                                                          // number
                                                                          // (short)
                                                       (byte) (GZIP_MAGIC >> 8), // Magic
                                                                                 // number
                                                                                 // (short)
        ...
        protected InputStream createContentStream() {
            contentStream =
                new InternalGzipCompressingInputStream(new CRC32InputStream(in),
                                                       bufferSize);
    ...
    protected static class InternalGzipCompressingInputStream
        ...
        public InternalGzipCompressingInputStream(CRC32InputStream in,
                                                  int bufferSize) {
            super(in,
        ...
        private void writeTrailer(byte[] buf, int offset) {
            writeInt((int) crcIn.getCrcValue(), buf, offset); // CRC-32 of
                                                              // uncompr. data
            writeInt((int) crcIn.getByteCount(), buf, offset + 4); // Number of
                                                                   // uncompr.
                                                                   // bytes
        }
        ...
        /*
         * Writes short integer in Intel byte order to a byte array, starting at
         * a given offset
         */
