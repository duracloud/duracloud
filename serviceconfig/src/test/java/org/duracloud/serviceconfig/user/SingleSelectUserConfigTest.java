 ...
 *         Date: Dec 5, 2010
 */
public class SingleSelectUserConfigTest {

    private SingleSelectUserConfig config;

    @Before
    public void setUp() {
        config = new SingleSelectUserConfig("name",
                                            "displayName",
                                            createOptions());
    }
    ...
    private void verify(SingleSelectUserConfig source,
        ...
        try {
            ...
            Assert.assertEquals(source.getSelectedOption(),
                                clone.getSelectedOption());
            Assert.assertEquals(source.getSelectedValue(),
                                clone.getSelectedValue());
            isValid = true;
